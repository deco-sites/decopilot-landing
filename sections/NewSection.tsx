  ...
  return (
    <div class="relative">
      ...
      <div class="absolute inset-0 flex flex-col items-center justify-center text-center p-4 md:p-8">
        ...
        <button class="bg-black text-white py-2 px-4 md:px-6 rounded-md">{ctaText}</button>
        <div class="mt-8 md:mt-12">
          <h3 class="text-lg md:text-2xl font-bold mb-2 md:mb-4">What are people saying?</h3>
          <div class="flex flex-col md:flex-row justify-center">
            {testimonials.map((testimonial, index) => (
              <div key={index} class="card bg-base-200 shadow-xl m-2 md:m-4 max-w-md">
                <div class="card-body">
                  <p class="text-base md:text-lg mb-2">{testimonial.text}</p>
                  <div class="card-actions justify-end">
                    <p class="text-sm md:text-base">{testimonial.author}, {testimonial.location}</p>
                  </div>
                </div>
              </div>
            ))}
